...
void PhysicsEngine::updateECS(Scene &scene, float deltaTime) {
  float damping = 0.5f;
  for (EntityID entity :
       SceneView<PositionComponent, VelocityComponent, AccelerationComponent,
                 CircleComponent>(&scene)) {
    auto positionComponent = scene.Get<PositionComponent>(entity);
    auto velocityComponent = scene.Get<VelocityComponent>(entity);
    auto accelerationComponent = scene.Get<AccelerationComponent>(entity);
...
void PhysicsEngine::update(std::vector<Object *> *objects, float deltaTime) {
  ...
  for (size_t i = 0; i < objects->size(); ++i) {
    for (size_t j = i + 1; j < objects->size(); ++j) {
      auto obj1 = (*objects)[i];
      auto obj2 = (*objects)[j];
      if (obj1->getType() == ObjectType::Circle &&
          obj2->getType() == ObjectType::Circle) {
      ...
      } else if (obj1->getType() == ObjectType::AABB &&
                 obj2->getType() == ObjectType::Circle) {
      ...
      } else if (obj1->getType() == ObjectType::Circle &&
                 obj2->getType() == ObjectType::AABB) {
      ...
      } else if (obj1->getType() == ObjectType::Polygon &&
                 obj2->getType() == ObjectType::AABB) {
        ...
        if (checkCollisionPolygonAABB(*polygon, *aabb)) {
          ...
          resolveCollisionPolygonAABB(manifold);
                 ...
                 obj2->getType() == ObjectType::Polygon) {
        auto *aabb = dynamic_cast<AABB *>(obj1);
...
bool PhysicsEngine::checkCollisionAABBCircle(const AABB &aabb,
  ...
  const float closestX =
      std::max(aabb.min.x, std::min(circle.position.x, aabb.max.x));
  const float closestY =
      std::max(aabb.min.y, std::min(circle.position.y, aabb.max.y));
...
void PhysicsEngine::resolveCollisionAABBCircle(Manifold &m) {
  ...
  float penetrationDepth =
      circle->radius - glm::length(circle->position - closestPoint);
...
bool PhysicsEngine::checkCollisionPolygonAABB(const Polygon &p,
  ...
  std::vector<glm::vec3> aabbVertices =
      calculateAABBvertices(aabb.min, aabb.max);
